    ...
    for filename in os.listdir(path):
        ...
        for i in range( len(data)):
            ...
            for triple in row_relation:
                ...
                tup = (sub,obj,sentence,len(row_relation),dist)
                lst_all.append(tup)
                rel = pred.split('/')
                rel = rel[len(rel) -1]
...
def create_rdf(output,dsName):
    ...
    g.add((URIRef(res+"Dataset_2"), # will go to all code
           #URIRef(prop+'name'),
           DC.title, 
           URIRef(res+dsName)
           ))
    g.add((URIRef(res+dsName),
           ...
           URIRef(res+'distant_supervision')
           ))
    g.add((URIRef(res+dsName),
           DC.source, 
           URIRef(res+'text_freebase')
           ))
    g.add((URIRef(res+dsName),
           URIRef(prop+'reType'), 
           URIRef(res+'ternary')
    ...
    for i, (rel,info) in enumerate(output.items()):
        rel = rel[0]
        print(rel)
        ID = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'Nrid'].iloc[0]
        print(ID)
        ...
        #       ))
        # end Testing code
        wr = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'WikidataIds'].iloc[0]
        if str(wr) != 'nan':
            g.add((URIRef(relation), # This property only is used for fewRel and wikire
                   OWL.sameAs, 
                   URIRef(wikidata+wr)
                   ))
        fb = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'freebase'].iloc[0]
        if str(fb) != 'nan':
            g.add((URIRef(relation), # freebase
              OWL.sameAs, 
              URIRef(freebase+fb)
             ))
        wikire = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'RE-WikiRE-Relation'].iloc[0]
        if str(wikire) != 'nan':
        ...
        few = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'RE-FewRel-Relation'].iloc[0]
        if str(few) != 'nan':
        ...
        # nyt = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'RE-NYT-Relation'].iloc[0]
        # if str(nyt) != 'nan':
        ...
        #      ))
        nlg = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'RE-WebNLG-Relation'].iloc[0]
        if str(nlg) != 'nan':
        ...
        google = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'RE-Google-Relation'].iloc[0]
        if str(google) != 'nan':
        ...
        equal = rel_data.loc[rel_data['RE-NYT-Relation'] == rel, 'owl:equivalentProperty'].iloc[0]
        if str(equal) != 'nan':
    ...
    g.serialize(destination="output/NYT/NYT_171221_final.ttl", format='ttl')
