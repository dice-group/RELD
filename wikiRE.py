...
def structured_data(path):
    ...
    for filename in os.listdir(path):
        ...
        with open(fullname) as f:
            ...
            for i in range(len(wikidata_corpus)):
                
                if i % 5000 ==  0:
                    print(f'Number of records processed: {i}')
                entities = {}
                ...
                for r_entry in wikidata_corpus[i]['edgeSet']:
                    ...
                    lst_all = []
                    tup = ()
        #relations[r_entry['kbID']] = (r_entry['left'],r_entry['right'])
                    ...
                    tup = (sub,obj,sentence,len(wikidata_corpus[i]['edgeSet']),dist)
                    lst_all.append(tup)
                    if pred in output:
...
def create_rdf(output,dsName,subject_dict,object_dict):
    ...
    g.add((URIRef(res+"Dataset_4"), # will go to all code
           #URIRef(prop+'name'),
           DC.title, 
           URIRef(res+dsName)
           ))
    ...
    for i, (rel,info) in enumerate(output.items()):
        ID = rel_data.loc[rel_data['WikidataIds'] == rel, 'Wrid'].iloc[0]
        ...
        if rel in list_Others:
            ...
            relation_text = rel_data.loc[rel_data['WikidataIds'] == rel, 'RE-WikiRE-Relation'].iloc[0] 
        ...
        #       ))
        # end Testing code
        #wr = rel_data.loc[rel_data['RE-WikiRE-Relation'] == rel, 'WikidataIds'].iloc[0]
        #if str(wr) != 'nan':
        ...
        #      ))
        few = rel_data.loc[rel_data['WikidataIds'] == rel, 'RE-FewRel-Relation'].iloc[0]
        if str(few) != 'nan':
...
def save():
    a_file = open("wikire_output_train.csv", "w")
    writer = csv.writer(a_file)
    for key, value in output.items():
        writer.writerow([key, value])
    print('Output now saved')
    a_file.close()   

def main():
   
    dataset_Name = "Wiki-RE" 
